...
def build_model(model, config):
    if model == 'ECAPA2':
        ...
        model = wavLMBase(model_name = "microsoft/wavlm-base-sv",sr = config['sample_rate'],  model_location = config['device'])
    elif model == 'wavLMBasePlus':
        model = wavLMBase(model_name = "microsoft/wavlm-base-plus-sv",sr = config['sample_rate'], model_location = config['device'])
    elif model == 'MFA_Conformer':
        model = MFA_Conformer(model_location = config['device'], ckpt = "/ocean/projects/cis220031p/mbaali/sv_benchmark/epoch=17_cosine_eer=0.86.ckpt", lr=0.001 )
    elif model == 'MFA_Conformer_CAARMA':
        model = MFA_Conformer(model_location = config['device'], ckpt = "/ocean/projects/cis220031p/mbaali/sv_benchmark/epoch=97_cosine_eer=0.83.ckpt", lr=0.0045 )
