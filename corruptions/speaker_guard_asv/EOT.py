...
class EOT(nn.Module):
    ...
    def forward(self, x_batch, x_tgt_batch, y_batch, EOT_num_batches=None, EOT_batch_size=None, use_grad=None):
        ...
        for EOT_index in range(EOT_num_batches):
            ...
            x_tgt_batch_repeat = x_tgt_batch.repeat(EOT_batch_size, 1, 1)
            if use_grad:
                x_batch_repeat.retain_grad()
            y_batch_repeat = y_batch.repeat(EOT_batch_size)
            decisions_EOT, scores_EOT = self.model(x_batch_repeat, x_tgt_batch_repeat) # scores or logits. Just Name it scores. (batch_size, n_spks)
