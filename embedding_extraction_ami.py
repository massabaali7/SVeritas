...
def parse_args():
    ...
    parser.add_argument("--mic", type=str, default="near_field", help="mic")
...
def main():
    ...
    mic = config['mic'] # near_field
    list_models = ['Redimnet', 'ECAPA'] #,'MFA_Conformer', 'MFA_Conformer_CAARMA', 'wavLMBase', 'wavLMBasePlus'] #'Redimnet', 'ECAPA', 
    ...
    for model_name in list_models:
        ...
        for batch in dataloader:
            utt_id = batch['file'][0] #batch['filename'][0].split(".wav")[0]         # e.g., 'xzMcjMdrLIo_0273'
            speaker_id = batch['speaker'][0]  # e.g., 'xzMcjMdrLIo'
            waveform = batch['audio']    # Replace this for one file with waveform,sr = torchaudio.load("file.wav")
            ...
            # # Save embeddings
            spk_dir = os.path.join(config['embeddings_dir'], config["dataset_name"], mic, speaker_id, model_name)
